    ...
    <div className="mb-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          ...
          <p className="font-medium text-slate-900">{formatDate(flight.departureDate)}</p>
        </div>
      </div>
      ...
      <div className="flex items-center relative py-6">
        ...
        <div className="text-center w-1/3">
          ...
          <p className="text-2xl font-semibold text-slate-900">{flight.departureTime}</p>
          <p className="text-sm font-medium mt-1 text-slate-700">{flight.origin.split('(')[0].trim()}</p>
          <p className="text-xs text-slate-500 mt-0.5">{flight.origin.match(/\(([^)]+)\)/)?.[1]}</p>
        </div>
        ...
        <div className="w-1/3 px-4">
          <div className="relative">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-2 text-xs font-medium text-slate-500 whitespace-nowrap">
              Voo direto • {calculateDuration(flight.departureTime, flight.arrivalTime)}
            </div>
          </div>
        </div>
        ...
        <div className="text-center w-1/3">
          ...
          <p className="text-2xl font-semibold text-slate-900">{flight.arrivalTime}</p>
          <p className="text-sm font-medium mt-1 text-slate-700">{flight.destination.split('(')[0].trim()}</p>
          <p className="text-xs text-slate-500 mt-0.5">{flight.destination.match(/\(([^)]+)\)/)?.[1]}</p>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between p-4 bg-slate-50 rounded-lg">
        <div>
          ...
          <p className="font-semibold text-slate-900">R$ {flight.seatPrice.toFixed(2)}</p>
        </div>
        <div className="flex items-center">
          ...
          <span className="ml-2 text-sm text-slate-600">Bagagem de mão incluída</span>
        </div>
      </div>
    </div>
  );
